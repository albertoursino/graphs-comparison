...
from python_files.Utility import ar_dir_path, sc_dir_path
...
def main():
    ...
    for s_city, s_attrs in tqdm(s_cities_graph.copy().nodes(True)):
        found = False
    ...
    Utility.save_plot(reduced_routes_graph,
                      ar_dir_path + "reduced_routes_plot.png", "Airline Routes (Reduced)",
                      len(reduced_routes_graph.nodes), len(reduced_routes_graph.edges))
    Utility.save_plot(s_cities_graph,
                      sc_dir_path + "reduced_sister_cities_plot.png", "Sister Cities (Reduced)",
                      len(s_cities_graph.nodes), len(s_cities_graph.edges))
